...
try:
    ...
    print("plan orchestrator import error")
...
class PlanOrchestrator:
    ...
    async def process_chat_update(self, message: str, components: List[str], current_plan: Dict[str, Any], tier: int, context: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            self.logger.error(f"Error in workout plan update orchestration: {str(e)}")
            raise
    ...
    def _validate_components(self, components: List[str], current_plan: Dict[str, Any]):
        ...
        for component in components:
            if component not in current_plan:
                raise ValueError(f"Component not found in workout plan: {component}")
    ...
    def _enrich_context(self, context: Dict[str, Any], current_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'fitnessGoals' in current_plan:
            enriched['goals'] = current_plan['fitnessGoals']
            
        # Add training context if available
        if 'trainingContext' in current_plan:
            enriched['training'] = current_plan['trainingContext']
            
        # Add nutrition plan if available
        if 'nutritionPlan' in current_plan:
            enriched['nutrition'] = current_plan['nutritionPlan']
            
        # Add progress metrics if available
        if 'progressMetrics' in current_plan:
            enriched['metrics'] = current_plan['progressMetrics']
            
        return enriched
        
    def _needs_foundation_update(self, components: List[str]) -> bool:
        """Check if foundation components need update"""
        foundation_components = {'trainingContext', 'fitnessGoals', 'progressMetrics'}
    ...
    async def process_single_component(self, message: str, component: str, current_plan: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Process updates for a single component"""
        try:
    ...
    def generate_chat_response(self, updated_components: List[str]) -> str:
        """Generate a response indicating which components were updated"""
        try:
            component_list = ", ".join(updated_components)
            return f"I've updated the following aspects of your workout plan: {component_list}. How else can I help improve your training program?"
        except Exception as e:
            self.logger.error(f"Error generating chat response: {str(e)}")
            return "I've made the requested updates to your workout plan. Would you like me to explain any changes or make further modifications?" 